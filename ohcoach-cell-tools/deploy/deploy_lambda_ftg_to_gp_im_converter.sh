...
export ZIP_FILE="ftg_to_gp_im_converter.zip"
...
zip -r -q ${ZIP_FILE} \
            ...
            ohcoach_cell_tools/common/web1_api_helper.py \
            ...
            ohcoach_cell_tools/publishers/gp_im_publisher.py \
            ohcoach_cell_tools/constants.py \
            ohcoach_cell_tools/service/lambda_function_ftg_to_gp_im_converter.py

aws s3 cp ${ZIP_FILE} ${S3_BUCKET}

aws lambda update-function-configuration --function-name ${FTG_TO_GP_IM_FUNCTION} --memory-size 2056 --timeout 300
aws lambda update-function-code --function-name ${FTG_TO_GP_IM_FUNCTION} --zip-file fileb://${ZIP_FILE}
